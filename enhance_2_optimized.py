...
def demo_gpu_acceleration():
    ...
    if gpu_enhancer.base_enhancer.images:
        ...
        print("❌ No images found for testing")#!/usr/bin/env python3
"""
Part 7
...
class MTGSelectiveInterface:
    ...
    def create_selection_controls(self):
        ...
        # MTG Presets
        preset_buttons = HBox([
            Button(description="🎨 Art", button_style='info', layout={'width': '80px'}),
            Button(description="📝 Text", button_style='info', layout={'width': '80px'}),
            Button(description="📛 Name", button_style='info', layout={'width': '80px'}),
            Button(description="⚫ Border", button_style='info', layout={'width': '80px'})
        ])
        ...
        selection_buttons = HBox([
            Button(description="👁️ Preview", button_style='info', layout={'width': '100px'}),
            Button(description="🎯 Create", button_style='primary', layout={'width': '100px'}),
            Button(description="🗑️ Clear", button_style='danger', layout={'width': '100px'}),
            Button(description="🤖 Auto", button_style='success', layout={'width': '100px'})
        ])
        ...
        return VBox([
            HTML("<h3>🔧 Selection Tools</h3>"),
            HBox([self.selection_mode, self.selection_name]),
            HTML("<h4>📋 MTG Card Presets</h4>"),
            preset_buttons,
            self.pos_controls,
    ...
    def create_enhancement_controls(self):
        ...
        preset_enh_buttons = HBox([
            Button(description="🎨 Art Settings", button_style='info', layout={'width': '120px'}),
            Button(description="📝 Text Settings", button_style='info', layout={'width': '120px'}),
            Button(description="🔄 Reset", button_style='warning', layout={'width': '80px'})
        ])
        ...
        return VBox([
            HTML("<h3>⚙️ Enhancement Settings</h3>"),
            preset_enh_buttons,
    ...
    def create_action_controls(self):
        ...
        action_buttons = HBox([
            Button(description="🔍 Preview Enhancement", button_style='success', 
                  layout={'width': '160px', 'height': '35px'}),
            Button(description="✅ Apply to Area", button_style='primary', 
                  layout={'width': '140px', 'height': '35px'}),
            Button(description="🚀 Apply All Areas", button_style='primary', 
                  layout={'width': '140px', 'height': '35px'}),
            Button(description="💾 Save Result", button_style='info', 
    ...
    def on_create_selection(self, b):
        ...
        if np.sum(mask) > 0:
            ...
            with self.selection_output:
                ...
                print(f"✅ Created selection '{name}': {pixels:,} pixels")
                self.show_current_selections()
        else:
            with self.selection_output:
                clear_output()
                print("❌ Empty selection - adjust parameters")
    ...
    def on_auto_detect(self, b):
        ...
        with self.selection_output:
            clear_output()
            print("🤖 Auto-detected 3 regions: art_area, text_area, name_area")
            self.show_current_selections()
    ...
    def on_preview_enhancement(self, b):
        ...
        if area_name not in self.current_selections:
            with self.preview_output:
                clear_output()
                print(f"❌ No selection made for {area_name}")
            return
    ...
    def on_apply_single(self, b):
        ...
        with self.preview_output:
            ...
            print(f"✅ Applied enhancement to {area_name}")
    ...
    def on_apply_all(self, b):
        ...
        with self.preview_output:
            ...
            print(f"✅ Applied enhancement to {len(self.current_selections)} areas")
    ...
    def on_save_result(self, b):
        ...
        if self.enhanced_result is None:
            with self.preview_output:
                clear_output()
                print("❌ No enhanced result to save. Apply enhancement first.")
            return
    ...
    def show_selection_preview(self, mask, title="Selection Preview"):
        ...
        print(f"📊 Selection covers {selected_pixels:,} pixels ({percentage:.1f}% of image)")
    ...
    def apply_enhancement_to_all(self):
        ...
        if not self.current_selections:
            print("❌ No selections to enhance")
            return
    ...
    def show_comparison_result(self, title="Enhanced Result"):
        ...
        if self.enhanced_result is None:
            print("❌ No enhanced result to show")
            return
    ...
    def save_enhanced_image(self):
        ...
        if self.enhanced_result is None:
            print("❌ No enhanced result to save")
            return
...
def demo_selective_enhancement():
    ...
    try:
        ...
        print("\n🚀 Creating selective enhancement interface...")
        ...
        print("\n💡 Quick Start Guide:")
        print("1. Click '🎨 Art' preset to select art area")
        print("2. Click '🎯 Create' to save the selection")
        print("3. Adjust enhancement sliders for art")
        print("4. Click '✅ Apply to Area'")
        print("5. Repeat for text area with different settings")
        print("6. Click '💾 Save Result'")
        
    except NameError:
        print("❌ Main MTG enhancer not found. Load the main code first.")
    except Exception as e:
        print(f"❌ Error: {e}")
